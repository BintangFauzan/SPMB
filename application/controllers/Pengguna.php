...
class Pengguna extends CI_Controller
{
	...
	public function tampil()
	{
		...
		$this->load->view('dashboard/pengguna', $d);
	...
	public function simpan()
	{
		...
		if ($id == 0) {
			...
			$insert = $this->pengguna->simpan("lps_login", $data);
		} else {
			$insert = $this->pengguna->update("lps_login", array('log_id' => $id), $data);
		}
		$error = $this->db->error();
		if (!empty($error)) {
			$err = $error['message'];
	...
	{
		$delete = $this->pengguna->delete('lps_login', 'log_id', $id);
